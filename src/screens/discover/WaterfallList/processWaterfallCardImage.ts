...
import { IMAGE_QUALITY_CONFIG, IMAGE_CONSTANTS } from './constants';
 ...
 */
export const getOptimizedImageUrl = (url: string, quality: ImageQuality): string => {
  if (!url) {
    return '';
  }
  
  const qualityParam = IMAGE_QUALITY_CONFIG[quality];
...
export const calculateImageDisplayHeight = (
  ...
  cardWidth: number = IMAGE_CONSTANTS.DEFAULT_WIDTH
): number => {
  // 如果没有尺寸信息，返回默认高度
  if (!width || !height || width <= 0 || height <= 0) {
    return IMAGE_CONSTANTS.DEFAULT_HEIGHT;
  }
  ...
  // 限制最小和最大高度，避免极端情况
  return Math.max(
    IMAGE_CONSTANTS.MIN_HEIGHT, 
    Math.min(IMAGE_CONSTANTS.MAX_HEIGHT, calculatedHeight)
  );
};
...
export const getPlaceholderImageUrl = (width: number = 300, height: number = 200): string => {
  return `https://via.placeholder.com/${width}x${height}/${IMAGE_CONSTANTS.PLACEHOLDER_COLOR.replace('#', '')}/999999?text=加载失败`;
};
