...
import { WATERFALL_CARD_FORMAT_THRESHOLDS } from './constants';
...
export const formatDisplayCount = (count: number): string => {
  if (count < WATERFALL_CARD_FORMAT_THRESHOLDS.THOUSAND) {
    return count.toString();
  }
  
  if (count < WATERFALL_CARD_FORMAT_THRESHOLDS.TEN_THOUSAND) {
    return `${(count / WATERFALL_CARD_FORMAT_THRESHOLDS.THOUSAND).toFixed(1)}k`;
  }
  
  return `${(count / WATERFALL_CARD_FORMAT_THRESHOLDS.TEN_THOUSAND).toFixed(1)}w`;
};
