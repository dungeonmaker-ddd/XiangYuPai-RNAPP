...
import type { UserCardType, FunctionItem, LocationInfo } from './home';

// ICON_MAP 已移动到 ./home/FunctionGrid.tsx

// 类型定义
// HomeScreen 现在作为独立页面，不需要导航相关Props
interface HomeScreenProps {}

// UserCard, FunctionItem, LocationInfo 类型已移动到 ./home/types.ts

// ══════════════════════════════════════════════════════════════
// 2. Constants & Config (40-60行)
// ══════════════════════════════════════════════════════════════
// SCREEN_WIDTH, SCREEN_HEIGHT, COLORS, GRID_CONFIG 已移动到 ./home/constants.ts
// FILTER_TABS 已移动到 ./home/FilterTabs.tsx

// FUNCTION_ITEMS 已移动到 ./home/FunctionGrid.tsx

// ══════════════════════════════════════════════════════════════
// 3. Utils & Helpers (60-80行)
// ══════════════════════════════════════════════════════════════
// formatDistance, truncateText, getStatusColor, getStatusText 已移动到 ./home/UserCard.tsx
...
// 根据筛选条件生成Mock数据
const generateMockUsers = (filter: string = 'nearby', region?: string): UserCardType[] => {
  const baseCount = 20;
  ...
  for (let i = 0; i < baseCount; i++) {
    ...
    // 根据筛选类型调整数据
    let status: UserCardType['status'] = 'offline';
...
const useHomeScreenState = () => {
  ...
  const [limitedOffers, setLimitedOffers] = useState<UserCardType[]>([]);
...
// HeaderSection 组件已拆分到 ./home/HeaderSection.tsx

// GameBanner 组件已拆分到 ./home/GameBanner.tsx

// FunctionGrid 组件已拆分到 ./home/FunctionGrid.tsx

// LimitedOffers 组件已拆分到 ./home/LimitedOffers.tsx

// TeamPartySection 组件已拆分到 ./home/TeamPartySection.tsx

// FilterTabs 组件已拆分到 ./home/FilterTabs.tsx

// UserCard 组件已拆分到 ./home/UserCard.tsx
...
const HomeScreen: React.FC<HomeScreenProps> = () => {
  ...
  const handleUserPress = useCallback((user: UserCardType) => {
  ...
  const handleMoreTeamPartyPress = useCallback(() => {
    // navigation.navigate('TeamPartyList');
    console.log('Navigate to team party list');
  }, []);
  ...
  const handleTeamPartyPress = useCallback(() => {
    // navigation.navigate('TeamCenter');
    console.log('Navigate to team center');
  }, []);
  ...
  const renderUserItem = useCallback(({ item }: { item: UserCardType }) => (
...
// UserCard, FunctionItem, LocationInfo, COLORS, GRID_CONFIG 已移动到 ./home/ 模块中
